          ...
          : !abierto
            ...
            : <div style={{ background:C.bgWarm, borderRadius:12, padding:16, border:`1px solid ${C.border}` }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  ...
                  <button onClick={confirmar} style={{ flex:1, background:C.brown, color:"#fff", border:"none", borderRadius:8, padding:"11px", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:sans }}>Confirmar reserva</button>
                </div>
              </div>
        }
      </div>
      ...
      <div style={{ background:C.brown, padding:"20px 20px 0" }}>
        <div style={{ maxWidth:560, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", gap:12, marginBottom:16 }}>
            {[[prendas.length,"Total"],[prendas.filter(p=>p.estado==="publicada").length,"Publicadas"],[prendas.filter(p=>p.estado==="borrador").length,"Borradores"],[prendas.filter(p=>p.estado==="reservada").length,"Reservadas"]].map(([n,label])=>(
...
export default function App(){
  ...
  const cambiarEstado = async (id, estado) => {
    await supabase.from('prendas').update({ estado, reserva_nombre: estado==="publicada"||estado==="borrador" ? null : undefined, reserva_cel: estado==="publicada"||estado==="borrador" ? null : undefined }).eq('id', id);
    cargarPrendas();
  };
